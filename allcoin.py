...
class autoTrade1 :
    ...
    def get_today_data1(self) :
        ...
        # 노이즈 1일 평균
        daily_data1['noise_ma20'] = daily_data1['noise'].rolling(window=2).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
    ...
    def get_today_data2(self) :
        ...
        # 노이즈 1일 평균
        daily_data2['noise_ma20'] = daily_data2['noise'].rolling(window=2).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        ...
        # 5일 이동평균선
        daily_data2['ma5'] = daily_data2['close'].rolling(window=15, min_periods=1).mean().shift(1)
        # 상승장 여부
    ...
    def get_today_data3(self) :
        ...
        # 노이즈 1일 평균
        daily_data3['noise_ma20'] = daily_data3['noise'].rolling(window=2).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        ...
        # 5일 이동평균선
        daily_data3['ma5'] = daily_data3['close'].rolling(window=15, min_periods=1).mean().shift(1)
        # 상승장 여부
    ...
    def get_today_data4(self) :
        ...
        # 노이즈 1일 평균
        daily_data4['noise_ma20'] = daily_data4['noise'].rolling(window=2).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        ...
        # 5일 이동평균선
        daily_data4['ma5'] = daily_data4['close'].rolling(window=15, min_periods=1).mean().shift(1)
        # 상승장 여부
    ...
    def get_today_data5(self) :
        ...
        # 노이즈 1일 평균
        daily_data5['noise_ma50'] = daily_data5['noise'].rolling(window=5).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        ...
        # 5일 이동평균선
        daily_data5['ma5'] = daily_data5['close'].rolling(window=15, min_periods=1).mean().shift(1)
        # 상승장 여부
    ...
    def get_today_data6(self) :
        ...
        # 노이즈 1일 평균
        daily_data6['noise_ma60'] = daily_data6['noise'].rolling(window=6).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        ...
        # 5일 이동평균선
        daily_data6['ma5'] = daily_data6['close'].rolling(window=15, min_periods=1).mean().shift(1)
        # 상승장 여부
    ...
    def get_today_data7(self) :
        ...
        # 노이즈 1일 평균
        daily_data7['noise_ma70'] = daily_data7['noise'].rolling(window=7).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        ...
        # 5일 이동평균선
        daily_data7['ma5'] = daily_data7['close'].rolling(window=15, min_periods=1).mean().shift(1)
        # 상승장 여부
    ...
    def get_today_data8(self) :
        ...
        # 노이즈 1일 평균
        daily_data8['noise_ma80'] = daily_data8['noise'].rolling(window=8).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        ...
        # 5일 이동평균선
        daily_data8['ma5'] = daily_data8['close'].rolling(window=15, min_periods=1).mean().shift(1)
        # 상승장 여부
    ...
    def get_today_data9(self) :
        ...
        # 노이즈 1일 평균
        daily_data9['noise_ma90'] = daily_data9['noise'].rolling(window=9).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        ...
        # 5일 이동평균선
        daily_data9['ma5'] = daily_data9['close'].rolling(window=15, min_periods=1).mean().shift(1)
        # 상승장 여부
    ...
    def get_today_data10(self) :
        ...
        # 노이즈 1일 평균
        daily_data10['noise_ma100'] = daily_data10['noise'].rolling(window=10).mean().shift(1)
        # 변동폭 ( 고가 - 저가 )
        ...
        # 5일 이동평균선
        daily_data10['ma5'] = daily_data10['close'].rolling(window=15, min_periods=1).mean().shift(1)
        # 상승장 여부
...
acc_key = "9CDpgxMa0ixn3hH5QRZvicWWK6t9o0TvZPrNOGMF"    # Access Key
sec_key = "7sPoqmRYogdtqObx8bUnrxDjGbsyD9CTYeigN2LI"    # Secret Key
app_token = "xoxb-3263374258582-3275294383764-1dMwA0PezIqW6SzEgCNQQ7WC"  # App Token
