...
class autoTrade1 :
    ...
    def start(self) :
        ...
        while True :
            try :
                ...
                end_time = start_time + datetime.timedelta(days=1)
                sell_time1 = start_time + datetime.timedelta(hours=23, minutes=59, seconds=56)
                sell_time2 = end_time - datetime.timedelta(seconds=1)
                ...
                if((current_price1 >= self.target_price1) and self.bull1 and not self.buy_yn1 and(start_time + datetime.timedelta(minutes=1) < now < end_time - datetime.timedelta(minutes=1))) : # 매수 시도
                    print("==================== [ 매수 시도 ] ====================")
                    self.buy_coin1()   
                if((current_price2 >= self.target_price2) and self.bull2 and not self.buy_yn2 and(start_time + datetime.timedelta(minutes=1) < now < end_time - datetime.timedelta(minutes=1))) : # 매수 시도
                    print("==================== [ 매수 시도 ] ====================")
                    self.buy_coin2()  
                if((current_price3 >= self.target_price3) and self.bull3 and not self.buy_yn3 and(start_time + datetime.timedelta(minutes=1) < now < end_time - datetime.timedelta(minutes=1))) : # 매수 시도
                    print("==================== [ 매수 시도 ] ====================")
                    self.buy_coin3()
                if((current_price4 >= self.target_price4) and self.bull4 and not self.buy_yn4 and(start_time + datetime.timedelta(minutes=1) < now < end_time - datetime.timedelta(minutes=1))) : # 매수 시도
                    print("==================== [ 매수 시도 ] ====================")
                    self.buy_coin4() 
                if((current_price5 >= self.target_price5) and self.bull5 and not self.buy_yn5 and(start_time + datetime.timedelta(minutes=1) < now < end_time - datetime.timedelta(minutes=1))) : # 매수 시도
                    print("==================== [ 매수 시도 ] ====================")
                    self.buy_coin5()  
                if((current_price6 >= self.target_price6) and self.bull6 and not self.buy_yn6 and(start_time + datetime.timedelta(minutes=1) < now < end_time - datetime.timedelta(minutes=1))) : # 매수 시도
                    print("==================== [ 매수 시도 ] ====================")
                    self.buy_coin6()
                if((current_price7 >= self.target_price7) and self.bull7 and not self.buy_yn7 and(start_time + datetime.timedelta(minutes=1) < now < end_time - datetime.timedelta(minutes=1))) : # 매수 시도
                    print("==================== [ 매수 시도 ] ====================")
                    self.buy_coin7()    
                if((current_price8 >= self.target_price8) and self.bull8 and not self.buy_yn8 and(start_time + datetime.timedelta(minutes=1) < now < end_time - datetime.timedelta(minutes=1))) : # 매수 시도
                    print("==================== [ 매수 시도 ] ====================")
                    self.buy_coin8()
                if((current_price9 >= self.target_price9) and self.bull9 and not self.buy_yn9 and(start_time + datetime.timedelta(minutes=1) < now < end_time - datetime.timedelta(minutes=1))) : # 매수 시도
                    print("==================== [ 매수 시도 ] ====================")
                    self.buy_coin9()  
                if((current_price10 >= self.target_price10) and self.bull10 and not self.buy_yn10 and(start_time + datetime.timedelta(minutes=1) < now < end_time - datetime.timedelta(minutes=1))) : # 매수 시도
                    print("==================== [ 매수 시도 ] ====================")
                    self.buy_coin10()                          
    ...
    def get_today_data2(self) :
        ...
        # 5일 이동평균선
        daily_data2['ma5'] = daily_data2['close'].rolling(window=5, min_periods=1).mean().shift(1)
        # 상승장 여부
    ...
    def get_today_data5(self) :
        ...
        # 5일 이동평균선
        daily_data5['ma5'] = daily_data5['close'].rolling(window=30, min_periods=1).mean().shift(1)
        # 상승장 여부
    ...
    def get_today_data6(self) :
        ...
        # 5일 이동평균선
        daily_data6['ma5'] = daily_data6['close'].rolling(window=10, min_periods=1).mean().shift(1)
        # 상승장 여부
    ...
    def get_today_data9(self) :
        ...
        # 5일 이동평균선
        daily_data9['ma5'] = daily_data9['close'].rolling(window=60, min_periods=1).mean().shift(1)
        # 상승장 여부
